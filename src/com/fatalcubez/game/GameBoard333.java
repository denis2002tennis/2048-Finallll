...
public class GameBoard333 {
	...
	public GameBoard333(int x, int y) {
		this.x = x;
		this.y = y;
		...
		scores = new ScoreManager333(this);
		scores.loadGame();
		if(scores.newGame()){
			start();
			scores.saveGame();
	...
	private void spawnRandom() {
		...
		while (notValid) {
			...
			Tile current = board[row][col];
			if (current == null) {
				int value = random.nextInt(5) < 4 ? 2 : 4;
	...
	public ScoreManager333 getScores(){
		return scores;
	}
}
