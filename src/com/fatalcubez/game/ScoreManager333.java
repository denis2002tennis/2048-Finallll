...
public class ScoreManager333 {
	...
	private String temp;
	private GameBoard333 gBoard;
	private boolean newGame;

	public ScoreManager333(GameBoard333 gBoard) {
		try {
	...
	private void createFile() {
		...
		try {
			...
			writer.write("" + 0);
			writer.newLine();
			for (int row = 0; row < GameBoard333.ROWS; row++) {
				for (int col = 0; col < GameBoard333.COLS; col++) {
					if(row == GameBoard333.ROWS - 1 && col == GameBoard333.COLS - 1){
						writer.write("" + 0);
					}
					else{
						writer.write(0 + "-");
	...
	public void saveGame() {
		...
		try {
			...
			for (int row = 0; row < GameBoard333.ROWS; row++) {
				for (int col = 0; col < GameBoard333.COLS; col++) {
					this.board[row * GameBoard333.COLS + col] = gBoard.getBoard()[row][col] != null ? gBoard.getBoard()[row][col].getValue() : 0;
					if (row == GameBoard333.ROWS - 1 && col == GameBoard333.COLS - 1)
						writer.write("" + board[row * GameBoard333.COLS + col]);
					else writer.write(board[row * GameBoard333.COLS + col] + "-");
				}
			}
			writer.close();
		} catch (Exception e) {
