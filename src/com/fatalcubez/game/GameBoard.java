...
public class GameBoard implements Fing {
	...
	public GameBoard(int x, int y) {
		...
		while(g==0) { g=1;
		audio.load("Моргенштерн - Новый Кадиллак.mp3", "BG");
		audio.adjustVolume("BG", -30);
	...
	private boolean move(int row, int col, int horizontalDirection, int verticalDirection, int direction) {
		...
		while (move) {
			...
			else if (board[newRow][newCol].getValue() == current.getValue() && board[newRow][newCol].canCombine()&ju||
					(Math.abs(numFib(board[newRow][newCol].getValue())-numFib(current.getValue()))==1&&!ju
					|| (numFib(board[newRow][newCol].getValue())==1 & numFib(current.getValue())==1))&& board[newRow][newCol].canCombine()&&!ju)
			{
				board[newRow][newCol].setCanCombine(false); if(ju) {
				board[newRow][newCol].setValue(board[newRow][newCol].getValue() * 2);}
				else {board[newRow][newCol].setValue(board[newRow][newCol].getValue()+current.getValue());}
				canMove = true;
				board[newRow - verticalDirection][newCol - horizontalDirection] = null;
				board[newRow][newCol].setSlideTo(new Point(newRow, newCol));
