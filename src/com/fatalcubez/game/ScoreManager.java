...
public class ScoreManager {
	...
	public ScoreManager(Fing bikeBoard) {
		try {
			filePath = new File("").getAbsolutePath();
		} catch (Exception e) {
			e.printStackTrace();
		}
		if(bikeBoard instanceof GameBoard) temp = "Normal.norm";
		else { temp="Tetris.norm"; k=false;}
        this.gBoard = bikeBoard; }
	...
	public void saveGame() {
		if(gBoard.getju()&gBoard instanceof GameBoard) temp="Normal.norm"; 
		else if(!gBoard.getju()&gBoard instanceof GameBoard) temp="Normal.fib";
		else if(!gBoard.getju()&gBoard instanceof BikeBoard) temp="Tetris.fib"; 
		else if(gBoard.getju()&gBoard instanceof BikeBoard) temp="Tetris.norm";
		...
		try {           
			...
			for (int row = 0; row < GameBoard.ROWS; row++) {
				for (int col = 0; col < GameBoard.COLS; col++) {
					...
					else {this.board[row * GameBoard.COLS + col] = (gBoard).getBoard()[row+1][col] != null ? (gBoard).getBoard()[row+1][col].getValue() : 0; }
					if (row == GameBoard.ROWS - 1 && col == GameBoard.COLS - 1)
	...
	public void loadGame() {
		try { if(gBoard.getju()&gBoard instanceof GameBoard) temp="Normal.norm"; 
		else if(!gBoard.getju()&gBoard instanceof GameBoard) temp="Normal.fib";
		else if(!gBoard.getju()&gBoard instanceof BikeBoard) temp="Tetris.fib"; 
		else if(gBoard.getju()&gBoard instanceof BikeBoard) temp="Tetris.norm"; 
