...
public class GameBoard555 {
	...
	public GameBoard555(int x, int y) {
		this.x = x;
		this.y = y;
		...
		scores = new ScoreManager555(this);
		scores.loadGame();
		if(scores.newGame()){
			start();
			scores.saveGame();
	...
	private boolean checkSurroundingTiles(int row, int col, Tile tile) {
		...
		if (col < COLS - 1) {
			Tile check = board[row][col + 1];
			if (check == null) return true;
			return tile.getValue() == check.getValue();
		}
		return false;
	}
	...
	private void spawnRandom() {
		...
		while (notValid) {
			...
			Tile current = board[row][col];
			if (current == null) {
				int value = random.nextInt(15) < 16 ? 2 : 4;
	...
	public ScoreManager555 getScores(){
		return scores;
	}
}
